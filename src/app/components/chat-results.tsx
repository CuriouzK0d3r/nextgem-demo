...
const ChatResults: React.FC<any> = ({
  submittedQuery,
  finished,
  setFinished,
  setChatMessage
  ...
  async function chat() {
    ...
    await fetch(DUMMY_URL, {
      ...
      if (response.ok && response.body) {
        const reader_tmp = response.body
          .pipeThrough(new TextDecoderStream())
          .getReader();
        setReader(reader_tmp);

        const readStream = (): any =>
          reader_tmp.read().then(({ value, done }) => {
              ...
              <div className=" flex-1 rounded-xl">
                <div className=" align-middle rounded inline-block w-full ">
                  <div
                    ...
                    className={
                      "rounded-lg object-cover object-center shadow-md shadow-blue-gray-900/50 " +
                      (submittedQuery.length
                        ? "border-t border-l border-r dark:border-gray-700 border-gray-200"
                        : "")
                    }
                  >
                    <div>
                      <div className="flex-1 justify-between flex flex-col h-[55rem]">
                        <div
                          id="messages"
                          className="flex flex-col space-y-4 p-3 h-full overflow-y-auto  scrollbar scrollbar-thumb-gray-900 scrollbar-track-gray-100 scrollbar-medium"
                        >
                              ...
                              <div key={index}>
                                <div className="flex items-end">
                                  <div className="flex flex-col space-y-2 text-md leading-tight max-w-xl mx-2">
                                    <div>
                                      <span
                                        className="px-4 py-3 rounded-xl inline-block text-white text-left"
                                        style={{ backgroundColor: "#6359E1" }}
                                      >
                                        {message}
                                      </span>
                                    </div>
                                ...
                                <div className="flex items-end float-right">
                                  <div className="flex flex-col space-y-2 text-md leading-tight text-left max-w-lg mx-2">
                                    <div>
                                      ...
                                      <span className="px-4 py-3 rounded-xl inline-block bg-blue-500 text-white">
                                        {message}
                                      </span>
                                    </div>
                                  </div>
                              ...
                              <div className="flex flex-col space-y-2 text-md leading-tight max-w-xl mx-2">
                                <div>
                                  {currentMessage?.length ? (
                                    <span
                                      style={{ backgroundColor: "#6359E1" }}
                                      className="px-4 py-3 rounded-xl inline-block text-white"
                                    >
                                      {currentMessage}
                                    </span>
                                  ) : (
